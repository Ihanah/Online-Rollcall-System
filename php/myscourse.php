...
$cid = $_GET['cid'];
$cn = mysqli_query($link, "SELECT course_name FROM professor where course_code = '$cid'");
$course_name = mysqli_fetch_row($cn);

$hisdlc = mysqli_query($link, "SELECT time FROM opcourse where course_code = '$cid'");
$result = mysqli_query($link, "SELECT * FROM opcourse where course_code = '$cid' order by date_time desc");

$hhisdlc = mysqli_query($link, "SELECT time FROM history where course_code = '$cid'");
$hresult = mysqli_query($link, "SELECT * FROM history where course_code = '$cid' order by date_time desc");
...
<head>
    <meta charset="UTF-8">
    <title>點名紀錄-<?php echo ''.$course_name[0].'('.$cid.')'?></title>
