...
$course = mysqli_query($link, "SELECT course_name FROM professor where account = '$account' && year = '$yearget' && semester = '$sem'");
$courseid = mysqli_query($link, "SELECT course_code FROM professor where account = '$account' && year = '$yearget' && semester = '$sem'");
//$cid = mysqli_fetch_row($courseid);
...
$dels = mysqli_query($link, "DELETE FROM rollcall where course_id = '$del'");
$del = mysqli_query($link, "DELETE FROM opcourse where id = '$del'");

@$hdel = $_GET['hdel'];
echo $hdel;
$hdel = mysqli_query($link, "DELETE FROM opcourse where id = '$hdel'");
$hdels = mysqli_query($link, "DELETE FROM rollcall where course_id = '$hdel'");


//把過期的課程放入history
while($row1 = mysqli_fetch_array($result)){
    $rs=mysqli_fetch_row($hisdlc);
    //$rt=mysqli_fetch_row($courses);
    $date1 = date ("Y-m-d H:i:s" , mktime(date('H')+6, date('i'), date('s'), date('m'), date('d'), date('Y'))); 
    $date2 = @$rs[0]; 
    $delcourse = @rt[0];
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    $clockc = $_POST["clockc"];
    $date2= $_POST["clockc"];
    $courcodec = mysqli_query($link, "SELECT course_code FROM professor where course_name = '$coursec' && account = '$account'");  
    $rs = mysqli_fetch_array($courcodec);
    $cid = @$rs[0];
    //echo $cid;
    ...
    if($stmt = mysqli_prepare($link, $sql)){
        $check_sql = "SELECT * FROM opcourse WHERE course_code = '$cid' " ;
        $check_rs = mysqli_query($link, $check_sql);
        $check = mysqli_fetch_array($check_rs);
                                       
            if($check == 0){   
                if($coursec=="請選擇"|| $classc=="請選擇" || $clockc==" "){
                                                        ...
                                                        <div class="container con">
                                                            <div style=' margin-bottom:20px; margin-left:10px;'>
                                                                <table class="table table-hover">
                                                                    ...
                                                                    <td>
                                                                        <select class="form-control" id="coursecid" name="coursec" onchange="a(this.id)">
